...
| **1 Sending engine** | Audited all 6 SMTP-send call sites → **2 physical engines** (`sender.ts` for campaigns/tests; inline nodemailer / `mailboxRuntime` for drip+portal). Found + fixed the real CONFLICTS (see below); documented the remaining duplications to consolidate. |
...
| **6 Launch wizard** | The **Deliverability Center** `/deliverability` + `GET /api/deliverability/checklist` IS the launch wizard: verdict + 12-point checklist (pass/fail/warn) + advisor with **exact blocker codes + actions** (never "unknown error"). Go-Live wizard also returns exact `topBlockers`. |
| **7 Sender rotation** | Audited: selection = `mailboxFleet.assignMailboxes` (queue-time, honors caps/health/anti-spray). **Warmup now enforced** (Task 3 writes `daily_send_limit`, which every cap check reads — previously `warmup_stage` was computed-but-discarded). Overload note: a campaign binds one mailbox — documented. |
...
**Consolidation (non-blocking, documented):** worker copies of `resolveSecret` lack the API's `ENV_REF_ALLOWED` allowlist (bounded — all live refs are valid mailbox refs; needs a shared package since workers can't import api/src); cap computation exists in 4 places over 3 data sources; `bounceProcessor` writes tenant `suppressions` only (all 3 send paths check `suppressions`, so still blocked in-tenant). None allow an unsafe send after the Stage-3 fixes.
